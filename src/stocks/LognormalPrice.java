...
public class LognormalPrice {
	...
	public void logprice(double So, double St, double mulog, double sdlog,
			double t) {
		...
		divisor = testval.isInfinite() ? Csmallnumber.getSmallnumber()
				: divisor;
		setPdf(floorvalue((Math.exp(-0.5
				* Math.pow(((Math.log(St) - getAverage()) / getSd()), 2)))
				* divisor));
	}

	public void returnrate(double exreturn, double volatility, double time) {
