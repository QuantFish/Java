...
public class YieldBisect extends IntervalBisection {
	...
	public YieldBisect(int Nofiterations, double Precision, double high,
			double low) {
	...
	// nominalstockprice (bond stock price): TODO: change this confusing
	// variable name to nominalpricevalue in beta 0.2
	protected double nominalstockprice;
	...
	// implements the abstract method from interval bisection
	{
		...
		poscashflow = rateperTerm;// cash flow out per term
		// as monthly amount * termperiod//
		solution = (poscashflow / rootinput * (1.0 - 1.0 / (Math.pow(
				1.0 + rootinput, rateindex))))
				+ (nominalstockprice / (Math.pow(1.0 + rootinput, rateindex)))
				- marketpricevalue;
		return solution;
	}

	public double yield(double noms, double term, double coupon, double mktp,
			double period) {
