...
public class Igamma extends ContinuedFraction {
	...
	public double errf(double xerr) {// error function for all x values positive
										// and negative//
		double aconst;
		double xvar;
		double sign = xerr;
	...
	public double errfc(double xerr) {// complimentary error func
		double aconst;
		double xvar;
		if (xerr == 0)
			return 1;
	...
	public double tricomi(double aval, double b)// little gamma if divided by
												// gamma a gives P(ax) lower
												// gamma ratio//
	{
		a = aval;
		x = b;
	...
	public double igamup(double aval, double bval)// R(a,x)if divided by gamma a
													// becomes Q(a,b) //
	{
		setInt(0);
		double fn;// R(a,x) //
	...
	public double igamrp(double aval, double valx)// computelower incomplete
	...
	public double igamrq(double aval, double valx)// computeupper incomplete
													// gamma function ratio Q//
	{
		setInt(0);
		double fn;// R(a,x) //
		double leadfactor;
	...
	private double leadhilo() {
		...
		return value;// returns lower or upper incomplete gamma lead
						// values.....not the ratio//
	}
	...
	public final double poisscumQ(int k, double xval)// x is expected mean, also
														// returns the prob that
														// p(x) is between 0 and
														// k-1 NOT at most k//
	{
		if (xval < 0)
			return 0;
	...
	public final double chisqrc(double chival, int fval)// Upper tail for x2<35
														// v<=30//
	{
	...
	public final double chisqrc2(double chival, int fval)// Upper tail for x2<35
															// v<=30//
	{
		double v = fval / 2.0;
		double kisqr = chival / 2.0;
