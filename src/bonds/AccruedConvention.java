 ...
 * date. The method getPreviousCouponDays returns the proportioned value for
 * the period last coupon to settlement, which is the portion of accrued time
 * due to a seller.
 */
...
public class AccruedConvention {
	...
	public double daycounts(int flagvalue, Calendar settlementdate,
		...
		previouscoupon.set(Calendar.MONTH,
				(nextcoupondate.get(Calendar.MONTH) - 6));
		...
		switch (flagvalue) {
		case 1:
			if (settlementdate.get(Calendar.MONTH) == nextcoupondate
					.get(Calendar.MONTH))
			// Actual/actual in period (eg, US gov)
			{
				samedays = (nextcoupondate.get(Calendar.DATE) - settlementdate
						.get(Calendar.DATE));
				for (int n = (previouscoupon.get(Calendar.MONTH) + 1); n < nextcoupondate
						.get(Calendar.MONTH); n++) {
					temp.set(Calendar.MONTH, n);
					actualdays += temp.getActualMaximum(Calendar.DAY_OF_MONTH);
				}
				actualdays += (previouscoupon
						.getActualMaximum(Calendar.DAY_OF_MONTH) - previouscoupon
						.get(Calendar.DATE));
				setPreviousCoupondays((double) (actualdays - samedays)
						/ (actualdays));
				return (double) samedays / actualdays;
			}
			int setdays = (settlementdate
					.getActualMaximum(Calendar.DAY_OF_MONTH) - settlementdate
					.get(Calendar.DATE));
			actualday = setdays;
			for (int i = (settlementdate.get(Calendar.MONTH) + 1); i < nextcoupondate
		...
		case 2:
			for (int n = (previouscoupon.get(Calendar.MONTH) + 1); n < settlementdate
					.get(Calendar.MONTH); n++) {
				temp.set(Calendar.MONTH, n);
				actualdays += temp.getActualMaximum(Calendar.DAY_OF_MONTH);
			}
			...
			// Actual/365 (eg,UK gov)
			{
				samedays = (nextcoupondate.get(Calendar.DATE) - settlementdate
						.get(Calendar.DATE));
				setPreviousCoupondays((double) (((365.0 / coupons) - samedays) / (365.0 / coupons)));
				// requires annual multiple of coupon rate
				return (double) (samedays / (365.0 / coupons));
				// returns 1/365 ths of the annual coupon rate
			}
			actualday = (settlementdate.getActualMaximum(Calendar.DAY_OF_MONTH) - settlementdate
					.get(Calendar.DATE));
			for (int i = (settlementdate.get(Calendar.MONTH) + 1); i < nextcoupondate
					.get(Calendar.MONTH); i++) {
				temp.set(Calendar.MONTH, i);
			...
			actualday += nextcoupondate.get(Calendar.DATE);
			System.out.println("Actual days between coupon and settlement =="
					+ actualdays);
		...
		case 3:
			...
			// Actual/365 or 366 in leap year (eg,
			{
				...
				total = (previouscoupon.getActualMaximum(Calendar.DAY_OF_YEAR) | nextcoupondate
						.getActualMaximum(Calendar.DAY_OF_YEAR)) == 366 ? 366
						: 365;
			...
			actualday += nextcoupondate.get(Calendar.DATE);
			int totaldays;
			totaldays = (previouscoupon.getActualMaximum(Calendar.DAY_OF_YEAR) | nextcoupondate
		...
		case 5:
			...
			// 30/360..one day only..(eg,US corporate)
			{
				...
				setPreviousCoupondays((double) ((360.0 / coupons) - samedays)
						/ (360.0 / coupons));
			...
			for (int i = (settlementdate.get(Calendar.MONTH) + 1); i < nextcoupondate
				...
				couponset = ((couponset == 31) & (dayset == 30)) ? 30
						: couponset;
				actualday += couponset;
			}
