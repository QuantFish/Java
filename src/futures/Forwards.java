...
/**
 * TODO: Delete the mock implementations of methods conintr and pVcont
 * (overloaded) and find or make their real implementations.
 */
public final class Forwards {
	...
	/**
	 * method to return the dollar interest value coefficient for the term of a
	 * repo rate
	 * 
	 * @param term
	 *            is the term in years (as decimal) greater than 1 day
	 * @param reporate
	 *            the current bank base rate/ federal funds rate
	 */
	 ...
	 */
	public static double delpriceNoinc(double spotprice, double maturity,
			double reporate) {
	...
	public static double fpriceNoinc(double spotprice, double maturity,
		...
		return (spotprice - (PresentValue.pVcont(reporate,
				(maturity - currentime), deliveryprice)));
	}
	...
	public static double fpriceInc(double spotprice, double maturity,
		...
		return ((spotprice - income) * (conintr(
				reporate[(reporate.length - 1)], (maturity - currentime))));
	}

	public static double fvalueInc(double spotprice, double maturity,
			double currentime, double reporate, double period, double dividend,
	...
	public static double fvalueInc(double spotprice, double maturity,
			double currentime, double[] reporate, double period,
	...
	public static double fvaluegen(double fprice, double delivprice,
			double maturity, double currentime, double reporate) {
		return ((fprice - delivprice) * PresentValue.pVcont(reporate,
				(maturity - currentime)));
	}

	public static double fpriceDyld(double spotprice, double maturity,
			double currentime, double reporate, double dividendyld) {
		return (spotprice * conintr((reporate - dividendyld),
				(maturity - currentime)));
	}

	public static double fvalueDyld(double spotprice, double maturity,
			double currentime, double reporate, double dividendyld,
			double deliveryprice) {
		return ((fpriceDyld(spotprice, maturity, currentime, reporate,
				dividendyld) - (deliveryprice)) * PresentValue.pVcont(reporate,
				(maturity - currentime)));
	}
}
