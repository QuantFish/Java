...
import core.Probnormal;

//Implementation of class to compute BlackScholes valuation and option sensitivities
public final class BlackScholes {
	...
	public void bscholEprice(double sprice, double strike, double volatility,
		...
		setcalle(((sprice * Math.exp((brate - rate) * time)) * probd1)
				- ((strike * Math.exp(-rate * time)) * probd2));
		...
		gamma = ((densityfunc * (Math.exp((brate - rate) * time))) / (sprice
				* volatility * Math.sqrt(time)));
		vega = ((sprice * Math.exp((brate - rate) * time)) * densityfunc * Math
				.sqrt(time));
		double thetaterm1 = ((brate - rate) * (sprice
				* Math.exp((brate - rate) * time) * probd1));
		double thetaterm2 = (rate * (strike * Math.exp(-rate * time)) * densityfunc2);
		thetac = (((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility) / (2 * Math
				.sqrt(time))) - thetaterm1 - thetaterm2);
	}

	private void dvalues(double sprice, double strike, double volatility,
			double time, double rate) {
		brate = crate == 0.0 ? 0.0 : (brate = crate != rate ? (rate - crate)
				: rate);
		d1 = ((Math.log(sprice / strike) + (brate + (volatility * volatility) * 0.5)
				* time) / (volatility * Math.sqrt(time)));
		d2 = (d1 - (volatility * Math.sqrt(time)));
	}

	public void setDelta(double sprice, double strike, double volatility,
			double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		double probd1 = 0.0;
	...
	public void setGamma(double sprice, double strike, double volatility,
			double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		double probd1 = 0.0;
	...
	public void setVega(double sprice, double strike, double volatility,
			double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		double probd1 = 0.0;
	...
	public void setTheta(double sprice, double strike, double volatility,
			double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		...
		double thetaterma = ((brate - rate) * (sprice
				* Math.exp((brate - rate) * time) * probd3));
	...
	public void setRho(double sprice, double strike, double volatility,
			double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		if (brate != 0.0) {
	...
	public void setElstic(double sprice, double strike, double volatility,
			double time, double rate) {
		bscholEprice(sprice, strike, volatility, time, rate);
	...
	public void setCarry(double sprice, double strike, double volatility,
			double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		carryc = (time * (sprice * Math.exp((brate - rate) * time)) * p
				.ncDisfnc(d1));
		carryp = (-time * (sprice * Math.exp((brate - rate) * time)) * p
				.ncDisfnc(-d1));
	}
	...
	public double getDeltac() {
		if (deltac > 0.0)
			...
			throw new IllegalStateException("INCORRECT DELTA PARAMETRS"
					+ deltac);
	}
