...
public class PutCallPosition {
	...
	public void callprof(double callpr, double exercisepr, double stockprice) {// single
																				// data//
	...
	public void callprof(double callpr, double exercisepr, double[] stockprice) {
		// series data//
	...
	public void putprof(double putpr, double exercisepr, double stockprice) {
		double lngputprofit = stockprice <= exercisepr ? (-putpr + (exercisepr - stockprice))
				: -putpr;
		setLputprof(lngputprofit);
		double shrtputprofit = stockprice <= exercisepr ? (putpr + (exercisepr - stockprice))
				: putpr;
	...
	public void putprof(double putpr, double exercisepr, double[] stockprice) {
		...
		int indx = 0;
		for (double s : stockprice) {
			lngputprofit[indx] = stockprice[indx] <= exercisepr ? (-putpr + (exercisepr - stockprice[indx]))
					: -putpr;
			shrtputprofit[indx] = stockprice[indx] <= exercisepr ? (putpr + (stockprice[indx] - exercisepr))
					: putpr;
			indx++;
		}
		setLputprof(lngputprofit);
