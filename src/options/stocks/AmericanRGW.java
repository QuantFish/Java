...
public class AmericanRGW extends IntervalBisection {
	...
	private double americanCall(double s, double x, double sigma, double t,
			double r) {
	...
	public double amCall(double s, double x, double sigma, double t, double r,
		...
		double a1 = (((Math.log((s - dividend * Math.exp(-rate * divtime)) / x) + ((rate + (sigma * sigma) / 2) * t)) / (sigma * Math
				.sqrt(t))));
		double a2 = (a1 - (sigma * Math.sqrt(t)));
		double b1 = (((Math.log((s - dividend * Math.exp(-r * divtime)) / si) + ((rate + (sigma * sigma) / 2) * divtime)) / (sigma * Math
				.sqrt(divtime))));
		...
		callvalue = ((s - dividend * Math.exp(-r * divtime)) * norm1
				+ (s - dividend * Math.exp(-r * divtime)) * m1 - x
				* Math.exp(-r * t) * m2 - (x - dividend)
				* Math.exp(-r * divtime) * norm2);
		return si;
	}
	...
	public static void main(String[] args) {
		...
		// higher dividend = lower the low value, lower the divi, higher the
		// upper guess
