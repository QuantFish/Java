package options.exotics.barrier;

/**
 * Copyright (C) 2013 QuantFish.com This source code is released under the
 * Artistic License 2.0. Code license:
...
public class BinaryBars {
	...
	public BinaryBars(double rate, double yield, double barlevel, int period) {
		r = rate;
		y = yield;
		barrier = barlevel;
		tau = period == 1 ? (1.0 / (24 * 365)) : period == 2 ? (1.0 / (365.0))
				: period == 3 ? (1.0 / (52.0)) : period == 4 ? (1.0 / (12.0))
						: 0.0;
	}
	...
	public double downInch(double asset, double cash, double volatility,
			double time) {
		h = barrier > asset ? (barrier * Math.exp(Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;// Correction for discrete time
										// (fromcontinuous)
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		if (asset <= h)
	...
	public double downIconc(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldic = s.barops(asset, strike, cash, volatility, time,
				13);
	...
	public double upIconc(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluicc = s.barops(asset, strike, cash, volatility, time,
				14);
	...
	public double downIaonc(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldiac = s.barops(asset, strike, cash, volatility, time,
				15);
	...
	public double upIaonc(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluiac = s.barops(asset, strike, cash, volatility, time,
				16);
	...
	public double downIconp(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldicp = s.barops(asset, strike, cash, volatility, time,
				17);
	...
	public double upIconp(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluicp = s.barops(asset, strike, cash, volatility, time,
				18);
	...
	public double downIaonp(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldiap = s.barops(asset, strike, cash, volatility, time,
				19);
	...
	public double upIaonp(double asset, double strike, double cash,
		...
		h = barrier < asset ? (barrier * Math.exp(-Math.sqrt(tau) * volatility
				* 0.5826)) : barrier;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluiap = s.barops(asset, strike, cash, volatility, time,
				20);
	...
	public double downOconc(double asset, double strike, double cash,
		...
		if (asset <= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldocc = s.barops(asset, strike, cash, volatility, time,
				21);
	...
	public double upOconc(double asset, double strike, double cash,
		...
		if (asset >= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluocc = s.barops(asset, strike, cash, volatility, time,
				22);
	...
	public double downOaonc(double asset, double strike, double cash,
		...
		if (asset <= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldoac = s.barops(asset, strike, cash, volatility, time,
				23);
	...
	public double upOaonc(double asset, double strike, double cash,
		...
		if (asset >= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluoac = s.barops(asset, strike, cash, volatility, time,
				24);
	...
	public double downOconp(double asset, double strike, double cash,
		...
		if (asset <= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldocp = s.barops(asset, strike, cash, volatility, time,
				25);
	...
	public double upOconp(double asset, double strike, double cash,
		...
		if (asset >= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluocp = s.barops(asset, strike, cash, volatility, time,
				26);
	...
	public double downOaonp(double asset, double strike, double cash,
		...
		if (asset <= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaldoap = s.barops(asset, strike, cash, volatility, time,
				27);
	...
	public double upOaonp(double asset, double strike, double cash,
		...
		if (asset >= h)
			return 0.0;
		BinaryBarrierOption s = new BinaryBarrierOption(r, y, h);
		double barriervaluoap = s.barops(asset, strike, cash, volatility, time,
				28);
